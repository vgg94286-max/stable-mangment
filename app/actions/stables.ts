...
// One row per active reservation, for the admin "export riders" feature.
// Riders with no active reservation are excluded entirely. A rider with
// multiple active reservations gets one row per horse/stable.
export type RiderExportRow = {
  rider_id: number
  rider_name: string
  horse_id: number
  horse_name: string
...
export async function getRiderReservationsExport(): Promise<RiderExportRow[]> {
  const session = await requireAdminSession()
  ...
  const rows = (await sql`
    ...
    JOIN stables s ON s.id = r.stable_id
    WHERE r.status = 'active'
    ORDER BY u.full_name, s.barn, s.number
  `) as RiderExportRow[]
  ...
  await logAdminAction(
    ...
    'Rider',
    null,
    `${session.name} exported the active riders list to Excel.`,
